...
- sap-bw-query: a successful deploy now writes two launch shortcuts to the user's visible desktop (which follows OneDrive/Known-Folder-Move redirection) so the studio — installed under the per-user, no-admin `%LOCALAPPDATA%\BWAutomationStudio` — is discoverable: `SAP BW Automation Studio (kein Passwortspeicher).lnk` (`eclipse.exe -noPwdStore`, recommended) and `SAP BW Automation Studio (mit Passwortspeicher).lnk` (no flag, so Eclipse's own secure store may remember credentials — manual login only; the automation/MCP surface still never handles passwords). Created via COM `WScript.Shell` (no admin, no deletion), refreshed each deploy, best-effort, and skipped in CI / when `BW_STUDIO_NO_SHORTCUT` is set.
- sap-bw-query: the builder additionally drops a small `.lnk` to the (local) published bundle ZIP on the visible desktop, so the finished ~1 GB build is visible and one-click accessible while only a few KB sync to OneDrive instead of the whole archive.
